    ...
    resource configuration[:owner] do
      resource ':id' do
        ...
        post "bsc/start_bsc" do
          ...
          @funding.funding_users.map {|e| Notification.create(notification_type: Notification.notification_type_project_value, content: content, user_id: e.user_id, is_read: false)}
          # 启动BSC后，投委会成员会收到对该项目的comments征集（提问）的邀请通知
          content = Notification.project_type_ask_to_review_desc.call(@funding.company.name)
          params[:investment_committee_ids].map {|e| Notification.create(notification_type: Notification.notification_type_project_value, content: content, user_id: e, is_read: false)}
        ...
        post "bsc/remind_to_vote" do
          ...
          if can? :remind, "to_vote"
            ...
            @funding.evaluations.where(is_agree: nil).map {|e| Notification.create(notification_type: Notification.notification_type_project_desc, content: content, user_id: e.user_id, is_read: false)}
          else
