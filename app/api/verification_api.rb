    ...
    get :verified do
      ...
      if can_verify_users != nil && can_verify_users.pluck(:user_id).include?(User.current.id)
        ...
        if Evaluation.find_by(user_id: User.current.id).nil?
          ...
          b = Verification.where(status: params[:status], verification_type: ["title_update", "ka_apply", "appointment_apply"])
          a = User.current.verifications.where(status: params[:status], verification_type: "bsc_evaluate")
