  ...
  mounted do
    resource configuration[:owner] do
      resource ':commentable_id' do
        ...
        get :comments do
          present Comment.where(commentable_type: configuration[:owner].classify, commentable_id: params[:commentable_id], type: params[:type].classify).paginate(page: params[:page], per_page: params[:per_page]), with: Entities::Comment
        end
