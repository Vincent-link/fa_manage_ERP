module Entities
  class MemberForNewsFeeds < Base
    ...
    expose :previous_position, if: {type: PaperTrail::Version.news_feeds_type_value("member_change_position")}, documentation: {type: 'integer', desc: '更改前职级'} do |_ins, opt|
      opt[:changes]["position_rank_id"].first
    end
    expose :following_position, if: {type: PaperTrail::Version.news_feeds_type_value("member_change_position")}, documentation: {type: 'integer', desc: '更改后职级'} do |_ins, opt|
